...
type Entry = {
  ...
  path: string;
  branch: { blocksList: { key: string; startHex: string | null; endHex: string | null }[] };
};
...
export default async function BlocksPage({ params }: { params: Promise<{ platform: string; game: string; region: string }> }) {
  const { platform, game, region } = await params;
  const entry = getEntry(platform, game, region);
  if (!entry) return null;
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      <h1 className="text-3xl font-bold tracking-tight text-foreground">Blocks — {entry.game.name} / {entry.region.name}</h1>
      <div className="mt-6 overflow-x-auto rounded-lg border border-border">
        <table className="w-full text-sm">
          ...
          <tbody>
            {entry.branch.blocksList.map((b) => (
              <tr key={b.key} className="border-t border-border/50">
                <td className="py-2 pl-4 pr-4 font-mono">
                  <Link className="text-primary hover:underline" href={`/${entry.path}/blocks/${encodeURIComponent(b.key)}`}>{b.key}</Link>
                </td>
                <td className="py-2 pr-4 font-mono">{b.startHex ?? '—'}</td>
                <td className="py-2 pr-4 font-mono">{b.endHex ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
