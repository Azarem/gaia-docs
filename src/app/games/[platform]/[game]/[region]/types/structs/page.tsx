...
export default async function TypesStructsPage({ params }: { params: Promise<{ platform: string; game: string; region: string }> }) {
  const { platform, game, region } = await params;
  const entry = getEntry(platform, game, region);
  if (!entry) return null;
  return (
    <div className="mx-auto max-w-4xl px-6 py-16">
      <h1 className="text-3xl font-bold tracking-tight text-foreground">Types — Structs — {entry.game.name} / {entry.region.name}</h1>
      <pre className="mt-6 overflow-x-auto rounded bg-muted p-3 text-xs text-foreground">{JSON.stringify(entry.branch.types?.structs ?? null, null, 2)}</pre>
